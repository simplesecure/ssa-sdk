    ...
    result = await retry(
      async (bail) => {
        ...
        if (response.status >= 400 && response.status <= 499) {
          bail(
            `__issueWebApiCmd failed with client error ${response.status} (${response.statusText})`
          );
          return;
        }
      ...
      },
      {
        ...
        onRetry: (error) => {
          log.warn(
            `Fetch attempt failed with error below. Retrying.\n${error}`
          );
        },
      }
    );
...
export async function __fetchNotifications(
  appId,
  renderNotifications,
  config,
  chatAddress
  ...
  if (notificationCheck) {
    ...
    if (address) {
      ...
      if (
        ...
        let activeNotifications = notificationData.filter(
          (a) => a.active === true
        );
        ...
        if (activeNotifications.length > 1) {
          ...
          const messagesSeen =
            localStorage.getItem(MESSAGES_SEEN) !== "undefined"
              ? JSON.parse(localStorage.getItem(MESSAGES_SEEN))
              : undefined;
          ...
          if (notificationsToReturn && notificationsToReturn.length > 0) {
            if (renderNotifications === true && !chatAddress) {
          ...
          if (
            renderNotifications &&
            !chatAddress &&
            notificationsToReturn.length > 0
          ) {
            ...
            localStorage.setItem(
              SIMPLEID_NOTIFICATION_FETCH,
              JSON.stringify(notificationsToReturn)
            );
            return notificationsToReturn;
          }
        }
...
function __loadMessages() {
  ...
  thirdDiv.innerHTML = messageData.notification
    ? messageData.notification.content
    : messageData.content; //TODO: this is a terrible hack
...
export async function __dismissMessages(appId, renderNotifications, config) {
  //First we get the notification id
  const messageData = JSON.parse(localStorage.getItem(ACTIVE_SID_MESSAGE));
  if (messageData) {
    const messageID = messageData.notification
      ? messageData.notification.id
      : messageData.id; //TODO: another terrible hack
    ...
    try {
      ...
      log.error(
        `Failed marking notifications as seen while dismissing messages.\n${suppressedError}`
      );
...
function __addPlainText(notificationsToReturn) {
  ...
  for (const notification of notificationsToReturn) {
    const plainText = notification.content.replace(/(<([^>]+)>)/gi, "");
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const filteredPosts = posts.filter(
        (p) => JSON.parse(p.message).message !== "CONVERSATION CLOSED"
      );
